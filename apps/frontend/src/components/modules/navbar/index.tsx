    ...
    return (
        <nav className="sticky z-50 top-0 flex flex-col bg-white border-b shadow shadow-gray-200/50 px-3">
            <header className="flex justify-between items-center w-full h-14">
                ...
                <button onClick={() => (document.getElementById('queueModal') as HTMLDialogElement).showModal()}
                    className="inline btn btn-sm bg-coral border-none text-white">
                    <span className='text-xl items-end'>{queueData.length}</span> คิว
                </button>
            </header>
            {path === '/home' && (
                                ...
                                {queueData.map((queue, index) => (
                                    <React.Fragment key={index}>
                                        <tr>
                                            ...
                                            <td>
                                                <p
                                                    className={`flex justify-center items-center w-6 h-6 border-2 rounded mx-auto ${queue.quantity > 6 ? 'border-red-600' :
                                                            queue.quantity > 3 ? 'border-yellow-300' :
                                                                'border-green-300'
                                                        }`}
                                                >
                                                    {queue.quantity}
